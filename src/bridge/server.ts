...
export async function startBridge(opts: BridgeOptions): Promise<Bridge> {
  ...
  // The OpenAI connector itself uses No authentication. The official
  // tunnel-client still authenticates to this loopback MCP endpoint with one
  // high-entropy credential owned by this Bridge runtime. It is never persisted
  // and naturally stops being useful when this Bridge exits.
  const tunnelAuthorizationToken = `c2c_tunnel_${randomBytes(32).toString("base64url")}`;
  const getTunnelAuthorization = (): TunnelAuthorization => ({
    value: `Bearer ${tunnelAuthorizationToken}`,
  });
  ...
  const tunnel =
    opts.tunnelProvider ??
    tunnelForInstallation(workspace.id, logger, getTunnelAuthorization, ownsInstallation);
  ...
  app.all(
    ...
    bearerAuth({
      ...
      internalBearer:
        tunnel.name === "openai-secure"
          ? {
              token: tunnelAuthorizationToken,
  ...
  app.post("/admin/revoke-all", adminGuard, (_req, res) => {
    ...
    logger.info(`Revoked all OAuth tokens (${count})`);
