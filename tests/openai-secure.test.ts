...
describe("OpenAI Secure Tunnel integration", () => {
  ...
  it("uses the official tunnel-client contract without persisting or exposing runtime secrets in argv", async () => {
    ...
    const tunnel = new OpenAiSecureTunnel({
      ...
      mcpAuthorization: () => ({ value: "Bearer local-runtime-secret" }),
      spawnProcess: (_binary, args, options) => {
        ...
        expect(options.env?.C2C_MCP_AUTHORIZATION).toBe("Bearer local-runtime-secret");
        expect(options.env?.OPENAI_ADMIN_KEY).toBeUndefined();
        expect(command).not.toContain("runtime-secret");
        expect(command).not.toContain("local-runtime-secret");
    ...
    expect(command).toContain("--mcp.extra-headers Authorization: env:C2C_MCP_AUTHORIZATION");
    const processState = fs.readFileSync(path.join(stateDir, "tunnel-client", "process.json"), "utf8");
    expect(processState).not.toContain("runtime-secret");
    expect(processState).not.toContain("local-runtime-secret");
    await tunnel.stop();
    expect(fs.readdirSync(path.join(stateDir, "tunnel-client"))).toHaveLength(0);
    expect(tunnel.status().running).toBe(false);
  });

  it("keeps one runtime authorization without timer-driven child replacement", async () => {
    const stateDir = makeTmpDir("openai-tunnel-stable-auth");
    dirs.push(stateDir);
    ...
    const tunnel = new OpenAiSecureTunnel({
      ...
      apiKey: "runtime-secret",
      binaryResolver: () => "/tmp/tunnel-client",
      mcpAuthorization: () => ({ value: "Bearer bridge-runtime-token" }),
      spawnProcess: (_binary, args, options) => {
    ...
    expect(headers).toEqual(["Bearer bridge-runtime-token", "Bearer bridge-runtime-token"]);
    expect(maxActiveChildren).toBe(1);
    expect(tunnel.status().running).toBe(true);
    await tunnel.stop();
  });

  it("recovers one exited client with the same Bridge authorization and current runtime key", async () => {
  ...
  it("enforces one tunnel-client owner per installation state", async () => {
    const stateDir = makeTmpDir("openai-tunnel-owner-state");
    dirs.push(stateDir);
    const makeTunnel = () =>
      new OpenAiSecureTunnel({
        stateDir,
